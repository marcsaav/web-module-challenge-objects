...
const burger = {name: "Burger", price: 18, category: "Lunch", discount: function(person) {
...
function createMenuItem(name, cost, category) {
  let newMenuItem = {name: name, price: cost, category: category }
  return newMenuItem
}

/* Task 1b: use your function to create 3 more menu items. You may add any items to the menu that you'd like */

const fries = createMenuItem("Fries", 8, "Lunch")
const sando = createMenuItem("Brekky Sando", 16, "Breakfast")
const soda = createMenuItem("Soda", 4, "Drinks")
...
function getReviewByRating(arr, rev) {
  let reviewRange = []
    for(let i = 0; i < arr.length; i++) {
      if(arr[i].rating >= rev && arr[i].rating < (rev + 1) ) {
...
function getLongReviews(arr) {
  ...
  for(i = 0; i < arr.length; i++) {
    let revLength = arr[i].feedback.split(" ").length;
    if(revLength > 15) {
...
function carMarker(odometer) {
   return car = {odometer:odometer, drive: function(distance) {
